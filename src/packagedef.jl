        ...
        while true
            ...
            elseif next_cmd.cmd == :debug
                ...
                try
                    ...
                    # A file that is not parseable Julia, or an expression in it
                    # that cannot be loaded, leaves nothing (more) to step
                    ...
                    # feeds. The run then ends the way a finished debuggee
                    # does, below. When this happens partway through, the
                    # client has just asked to continue or step, so it is not
                    # waiting at a stop, just as at a normal end of the code.
                    err isa DebugEngines.UserCodeError || rethrow()
                    DAPRPC.send(
                        endpoint,
